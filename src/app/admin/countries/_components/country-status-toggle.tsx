...
import { updateCountryDisabledStatus } from "@/actions/countries"
import { cn } from "@/lib/utils"

interface CountryStatusToggleProps {
  countryId: string
  countryName: string
  isDisabled: boolean
}

export function CountryStatusToggle({
  countryId,
  countryName,
  isDisabled,
}: CountryStatusToggleProps) {
  ...
  const applyChange = (disable: boolean) => {
    startTransition(async () => {
      await updateCountryDisabledStatus(countryId, disable)
      ...
      <Dialog open={showWarning} onOpenChange={setShowWarning}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogTitle>Disable {countryName}?</DialogTitle>
            <DialogDescription>
              Disabling this country will hide it from the public-facing visa
              selector and prevent new applications targeting it. Existing
              applications will not be affected. You can re-enable it at any
              time.
            </DialogDescription>
          </DialogHeader>
