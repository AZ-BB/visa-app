...
function TravellerFields({ idPrefix }: { idPrefix: string }) {
  return (
    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
      <div className="lg:col-span-2">
        ...
        <Accordion
          ...
          className="space-y-3"
        >
          <AccordionItem value="traveller-1">
            <AccordionTrigger className="text-primary-copy font-bold">
              Traveller #1
            </AccordionTrigger>
            <AccordionContent>
              <TravellerFields idPrefix="t1" />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="traveller-2">
            <AccordionTrigger className="text-primary-copy font-bold">
              Traveller #2
            </AccordionTrigger>
            <AccordionContent>
              <TravellerFields idPrefix="t2" />
            </AccordionContent>
          </AccordionItem>
          {travellerCount > 2 &&
            Array.from({ length: travellerCount - 2 }, (_, i) => i + 3).map(
              (n) => (
                <AccordionItem key={n} value={`traveller-${n}`}>
                  <AccordionTrigger className="text-primary-copy font-bold">
                    Traveller #{n}
                  </AccordionTrigger>
                  <AccordionContent>
                    <TravellerFields idPrefix={`t${n}`} />
                  </AccordionContent>
                </AccordionItem>
              )
            )}
        ...
        <div className="mt-6 flex items-center gap-3">
          <button
            type="button"
            onClick={() => setTravellerCount((c) => c + 1)}
      ...
      <ApplicationSidebar travellerCount={travellerCount} />
    </div>
  );
}
