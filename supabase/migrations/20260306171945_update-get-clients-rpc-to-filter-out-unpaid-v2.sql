-- RPC to get clients (profiles) with applications count, paginated and sortable
...
BEGIN
  ...
  -- Build ORDER BY - for name we need first_name, last_name
  ...
  -- Get total count with same filters (all clients; has_applications filters by paid apps only)
  EXECUTE format(
    $count$
    SELECT count(*)::bigint
    FROM (
      SELECT p.id
      FROM profiles p
      LEFT JOIN applications a ON a.profile_id = p.id AND a.is_paid = true
  ...
  -- Get paginated data with applications count (app_count = paid apps only)
  EXECUTE format(
    $data$
