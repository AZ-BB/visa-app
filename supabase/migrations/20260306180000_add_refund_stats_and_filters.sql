-- Drop 13-param list_applications_admin before creating 14-param version
DROP FUNCTION IF EXISTS list_applications_admin(int, int, text, text, uuid, text, text, text, text, boolean, uuid);

-- Update get_application_counts: add total_paid, refunded_amount, total_revenue (paid apps only)
...
BEGIN
  SELECT count(*)::bigint INTO v_total FROM applications WHERE is_paid = true;

  SELECT count(*)::bigint INTO v_in_progress
  FROM applications
  WHERE status = 'IN_PROGRESS' AND is_paid = true;
  ...
  FROM applications WHERE is_paid = true;
...
-- Update list_applications_admin: add amount_refunded_cents to SELECT, add p_refunded_filter
