-- Add p_profile_id to filter applications by client (profile)
-- When provided, only applications for that profile are returned
...
BEGIN
  ...
  -- Get total count with same filters (including profile_id when provided)
  EXECUTE format(
    $count$
    SELECT count(*)::bigint
    FROM applications a
