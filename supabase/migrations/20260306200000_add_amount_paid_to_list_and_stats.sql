-- Add amount_paid_cents to list_applications_admin
-- Update get_application_counts to use amount_paid for Total Paid and Revenue
...
BEGIN
  ...
  -- Use amount_paid_cents when available, else total_fee
  SELECT COALESCE(sum(
    CASE
      WHEN amount_paid_cents IS NOT NULL THEN amount_paid_cents / 100.0
      ELSE total_fee
    END
  ), 0)::numeric INTO v_total_paid
  FROM applications WHERE is_paid = true;
...
-- Add amount_paid_cents to list_applications_admin SELECT
